"use client";

import { useState, useEffect } from "react";
import Link from "next/link";
import { Volume2, VolumeX } from "lucide-react";

function TwitterIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className}>
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  );
}

function GitHubIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className}>
      <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
    </svg>
  );
}

function DiscordIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className}>
      <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
    </svg>
  );
}

const socialLinks = [
  { href: "https://twitter.com", label: "Twitter", Icon: TwitterIcon },
  { href: "https://discord.gg/olioreserve", label: "Discord", Icon: DiscordIcon },
];

const AUDIO_KEY = "olio-audio-enabled";

function useHeaderAudio() {
  const [enabled, setEnabled] = useState(true);

  useEffect(() => {
    try {
      const stored = localStorage.getItem(AUDIO_KEY);
      if (stored !== null) setEnabled(stored !== "false");
    } catch {}
  }, []);

  const toggle = () => {
    setEnabled((prev) => {
      const next = !prev;
      try { localStorage.setItem(AUDIO_KEY, String(next)); } catch {}
      // Dispatch storage event so GameProvider picks up the change
      window.dispatchEvent(new StorageEvent("storage", { key: AUDIO_KEY, newValue: String(next) }));
      return next;
    });
  };

  // Listen for changes from GameProvider side
  useEffect(() => {
    const handler = (e: StorageEvent) => {
      if (e.key === AUDIO_KEY && e.newValue !== null) {
        setEnabled(e.newValue !== "false");
      }
    };
    window.addEventListener("storage", handler);
    return () => window.removeEventListener("storage", handler);
  }, []);

  return { enabled, toggle };
}

export function Header() {
  const { enabled: audioEnabled, toggle: toggleAudio } = useHeaderAudio();

  return (
    <header className="sticky top-0 z-10 border-b border-white/10 bg-white/10 backdrop-blur-[1px]">
      <div className="relative mx-auto flex h-14 max-w-7xl min-w-0 items-center justify-between gap-2 px-3 sm:px-4 md:px-6">
        {/* Left: Logo + Nav */}
        <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-4 md:gap-8">
          <Link
            href="/"
            className="relative flex shrink-0 items-center transition-opacity hover:opacity-90"
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src="/RefineryLogo.png"
              alt="OIL"
              className="h-8 w-auto max-w-[140px] object-contain sm:h-9"
            />
          </Link>
          <nav className="flex shrink-0 items-center gap-2 sm:gap-4 md:gap-6">
            <Link
              href="/about"
              className="whitespace-nowrap text-xs font-medium text-white transition-colors hover:text-black sm:text-sm"
            >
              About
            </Link>
            <Link
              href="/explore"
              className="whitespace-nowrap text-xs font-medium text-white transition-colors hover:text-black sm:text-sm"
            >
              Explore
            </Link>
            <Link
              href="/docs"
              className="whitespace-nowrap text-xs font-medium text-white transition-colors hover:text-black sm:text-sm"
            >
              Docs
            </Link>
          </nav>
        </div>

        {/* Center: RefineryHeader image - hidden on narrow screens */}
        <div className="absolute left-1/2 top-1/2 hidden -translate-x-1/2 -translate-y-1/2 lg:block">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src="/RefineryHeader.png"
            alt="The Refinery"
            className="h-9 w-auto object-contain"
          />
        </div>

        {/* Right: Social + Wallet */}
        <div className="flex shrink-0 items-center gap-1 sm:gap-4">
          <div className="hidden items-center gap-2 sm:flex">
            <button
              type="button"
              onClick={toggleAudio}
              className="flex h-9 w-9 items-center justify-center rounded-lg text-white transition-colors hover:bg-white/5 hover:text-black"
              aria-label={audioEnabled ? "Mute audio (M)" : "Unmute audio (M)"}
            >
              {audioEnabled ? (
                <Volume2 className="h-5 w-5" strokeWidth={1.5} />
              ) : (
                <VolumeX className="h-5 w-5" strokeWidth={1.5} />
              )}
            </button>
            {socialLinks.map(({ href, label, Icon }) => (
              <a
                key={label}
                href={href}
                target="_blank"
                rel="noopener noreferrer"
                className="flex h-9 w-9 items-center justify-center rounded-lg text-white transition-colors hover:bg-white/5 hover:text-black"
                aria-label={label}
              >
                <Icon className="h-5 w-5" />
              </a>
            ))}
          </div>
          <button
            type="button"
            onClick={() => {}}
            className="rounded-lg bg-black px-3 py-2 text-xs font-medium text-white transition-all hover:bg-zinc-800 active:scale-[0.98] sm:px-4 sm:text-sm"
          >
            <span className="hidden sm:inline">Connect Wallet</span>
            <span className="sm:hidden">Connect</span>
          </button>
        </div>
      </div>
    </header>
  );
}
